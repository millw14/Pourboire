...
/**
 * The animated CPPN background.
 *
 * Split out of the hero and loaded on demand, because three.js plus
 * @react-three/fiber is roughly 600KB that the landing page otherwise had to
 * download and parse before it could show a headline.
 *
 * The fragment shader evaluates a small neural network **per pixel, per frame**.
 * At the default `frameloop="always"` with `dpr={[1, 2]}` that ran flat out on a
 * retina display forever -- including while scrolled past, and while the tab sat
 * in the background. The gating below is not a nicety; it is the difference
...
declare module '@react-three/fiber' {
  interface ThreeElements {
    cPPNShaderMaterial: {
      ref?: React.Ref<CPPNMaterial>;
      side?: THREE.Side;
    };
  }
}

interface CPPNMaterial {
  iTime: number;
...
function ShaderPlane({ paused }: { paused: boolean }) {
  ...
  useFrame((state) => {
    ...
    material.iResolution.set(state.size.width, state.size.height);
    // With frameloop="demand" nothing schedules the next frame unless we ask for
    // one, so the loop stops the moment `paused` flips.
...
/** False while the tab is hidden -- a background tab should not run a shader. */
...
/** True while any part of `ref` is within a screen of the viewport. */
...
export default function ShaderBackground() {
  ...
  // WebGL is unavailable in some browsers, VMs and locked-down environments.
  // Returning null falls through to the CSS gradient the hero paints underneath,
  // rather than leaving a black rectangle.
  if (failed) return null;

  return (
    <div ref={hostRef} className="absolute inset-0 h-full w-full" aria-hidden>
      <Canvas
        camera={{ position: [0, 0, 1], fov: 75, near: 0.1, far: 1000 }}
        gl={{ antialias: false, alpha: false, powerPreference: 'low-power' }}
        // Capped at 1.5 rather than 2: this shader is fill-rate bound, and dpr 2
        // costs 78% more pixels for no visible gain at this level of blur.
        dpr={[1, 1.5]}
